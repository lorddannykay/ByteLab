...
// Form submission (disabled - contact form converted to quiz)
// document.getElementById('contactForm').addEventListener('submit', function (e) {
//    e.preventDefault();
//    // ... form submission code removed for course template
// });
...
if (typeof jQuery !== 'undefined') {
    (function($) {
        ...
        var currentStage = 1;
        var totalStages = 5;
        var quizAnswers = { q1: 'A', q2: 'B', q3: 'B' };
        var userAnswers = {};
        ...
        function goToStage(stage) {
            ...
            showStage(stage);
            updateProgress();
            localStorage.setItem('bio_tech_stage', stage);
        ...
        $('#submit-quiz').on('click', function() {
            ...
            if (score >= 66) {
                resultHtml += '<p style="color: var(--primary-green); font-weight: 600; margin-top: 10px;">Congratulations! You passed! Your certificate is now available.</p>';
                $('#certificate-section').show();
                var name = localStorage.getItem('bio_tech_name') || 'Learner';
                $('#cert-name').text(name);
            } else {
                resultHtml += '<p style="color: #fff; margin-top: 10px;">Please review the course and try again. You need at least 66% to pass.</p>';
            }
        ...
        $('#download-cert').on('click', function() {
            ...
            var certUrl = 'certificate.html?name=' + encodeURIComponent(name) + '&date=' + encodeURIComponent(date) + '&course=Bio-Inspired Technology';
            window.open(certUrl, '_blank');
        });
        
        // Load saved progress
        var saved = localStorage.getItem('bio_tech_stage');
        if (saved) {
